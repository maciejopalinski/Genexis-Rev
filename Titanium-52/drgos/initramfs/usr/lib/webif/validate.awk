...
$1 == "hexdec" {
        valid_type = 1
        if ((value != "") && (value !~ /^0[xX][0-9a-fA-f]*$/)) { valid = 0; verr = "@TR<<Invalid value>>" }
}
...
{
  # Strip existing checksum from PIN
  checksum = pin % 10
  pin = int(pin / 10)
  accum = 0
  ...
  pin = pin * 10
  accum += 3 * (int(pin / 10000000) % 10)
  accum += 1 * (int(pin / 1000000) % 10)
  accum += 3 * (int(pin / 100000) % 10)
  accum += 1 * (int(pin / 10000) % 10)
  accum += 3 * (int(pin / 1000) % 10)
  accum += 1 * (int(pin / 100) % 10)
  accum += 3 * (int(pin / 10) % 10)
...
#phone pattern. The pattern can contain only digits, x or X.
$1 == "phpattern" {
	valid_type = 1
	if ((value != "") && (value !~ /^[_]?[0-9xX]*$/)) {
		valid = 0
		verr = "@TR<<Invalid value>>"
	}
}
	...
	if (value != "") {
		...
		for (i = 1; i <= n; i++) {
			if ((ports[i] !~ /^[[:digit:]]{1,5}$/) && (ports[i] !~ /^[[:digit:]]{1,5}-[[:digit:]]{1,5}$/)) {
				valid = 0
			} else if ((ports[i] ~ /^[[:digit:]]{1,5}$/) && ((ports[i] < 1) || (ports[i] > 65535))) {
				valid = 0
			} else if (ports[i] ~ /^[[:digit:]]{1,5}-[[:digit:]]{1,5}$/) {
				split(ports[i], ranges, "-")
				if ((ranges[1] < 1) || (ranges[1] > 65535) || (ranges[2] < 1) || (ranges[2] > 65535) || (ranges[1] >= ranges[2])) {
					valid = 0
				}
			}
        		if (valid == 0) verr = "@TR<<Invalid value>>"
		...
		if (options[i] == "required") {
			...
			if ($1 == "int") {
				...
				if (length(value) < min) { valid = 0; verr = "@TR<<String too short>> (@TR<<minimum length>>: " min ")"}
                                ...
                                if (length(value) < min) { valid = 0; verr = "@TR<<String too short>> (@TR<<minimum length>>: " min-2 ")"}
                        }
			...
			if ($1 == "int") {
				...
				if (length(value) > max) { valid = 0; verr = "@TR<<String too long>> (@TR<<maximum length>>: " max ")" }
                                ...
                                if (length(value) > max) { valid = 0; verr = "@TR<<String too long>> (@TR<<maximum length>>: " max-2 ")" }
                        }
