...
$1 ~ /^start_form/ {
	...
	print "<table width=\"96%\" summary=\"Settings\">"
	form_help = ""
	form_help_link = ""
}
$1 ~ /^field/ {
...
# 
($1 ~ /^progressbar/) {
	print "<div class=\"progressbar\" style=\"width:" $4 "px; height:15px\">"
	if ($8 == "")
	print "	<span class=\"progress\" style=\"width:" $5 "; height:13px\">" $6 "</span>"	
	else
	print "	<span class=\"progress-"$8"\" style=\"width:" $5 "; height:13px\">" $6 "</span>"	
	print "</div>"
	#show caption
	if ($3 != "" ) print "<em>" $3 "</em>"
}
$1 ~ /^text$/ { 	
	...
	if ($6 == "readonly")
        print cols "<input id=\"" $2 "\" type=\"text\" name=\"" $2 "\" value=\"" $3 "\" readonly=\"" $6 "\" size=\"" $7 "\" />" $4 cols_end
        else
	print cols "<input id=\"" $2 "\" type=\"text\" name=\"" $2 "\" value=\"" $3 "\" size=\"" $7 "\" />" $4 cols_end
}
...
$1 ~ /^reset/ { print "<input type=\"reset\" name=\"" $2 "\" value=\"" $3 "\" />" }
