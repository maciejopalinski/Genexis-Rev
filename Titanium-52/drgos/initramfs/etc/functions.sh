...
# Used to exclusively access /config flash filesystem.
# Each module wanting to access /config need first call fs_lock() to get
# the lock, then do read/write stuff in /config, when finished call fs_unlock()
# to release the lock.
fs_lock() {
    maxwait="$1"
    [ -z "$maxwait" ] && maxwait=86400 # Default to 24 hours
    LOCK=/var/lock/fs_config
    # Block to wait $1 seconds before we get a lock:
    lock -t $maxwait $LOCK 2>/dev/null
	if test "$?" != "0"; then
	  return 1
	fi
    mount -o remount,rw /config 2>/dev/null
    return 0
}
...
fs_unlock() {
    LOCK=/var/lock/fs_config
    if [ -f $LOCK ]; then
	  mount -o remount,ro /config 2>/dev/null
