...
#   none
#
header "Status" "Diagnostics" "@TR<<Diagnostics>>" ''
...
formtag_begin|ping_button|$SCRIPT_NAME
field|
text|ping_hostname|$FORM_ping_hostname
submit|ping_button|@TR<<Ping>>
formtag_end
helpitem|Ping
helptext|Helptext Ping#Will attempt to ping the defined host. This tests DNS and network connectivity.
formtag_begin|tracert_button|$SCRIPT_NAME
field|
text|tracert_hostname|$FORM_tracert_hostname
submit|tracert_button|@TR<<TraceRoute>>
formtag_end
helpitem|Traceroute
helptext|Helptext Traceroute#Uses traceroute utility to contact the defined host. This is useful for testing routing.
end_form
EOF
...
#	equal $_pid "0" && _pid=$(echo $ps_results | cut -d ' ' -f 1 | sed 2,99d)  # newer busybox
	_pid=$(echo $ps_results | cut -d ' ' -f 1 | sed 2,99d)  # newer busybox
	...
	#	echo "<br />Error: Utility terminated too quick.<br />"
		echo "<br />"
		echo "<pre>"
		cat  $tmpfile
		echo "</pre>"
